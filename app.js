...
function makeCard(x){
 ...
 b.innerHTML=(x.urgent?'<span class="urgent-tape" aria-hidden="true"></span>':'')+'<span class="draghint">↕</span><span class="card-status">'+esc(x.order_status||"Planned")+'</span><b>Move #'+esc(x.move_number??"—")+' · Job '+esc(x.job_number)+'</b><small>'+esc(x.origin)+' → '+esc(x.destination)+'</small><small>'+x.pallet_count+' pallets</small><small>Built by '+esc(x.created_by_name)+'</small>';
